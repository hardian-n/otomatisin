...
const InboxChannels = ({
  ...
  const load = useCallback(async () => {
    const data = await (await fetch('/inbox/threads/channels')).json();
    return data?.channels || [];
  }, [fetch]);

  const { data } = useSWR('inbox-threads-channels', load, {
  ...
  const normalizedIntegrations = useMemo(() => {
    return (data || []).map((integration: IntegrationLite) => ({
      ...integration,
...
export const InboxComponent = () => {
  ...
  const loadReplies = useCallback(
    async (path: string) => {
      return (await (await fetcher(path)).json()) as InboxResponse;
    },
    [fetcher]
  );
    ...
    <>
      ...
      <div className="bg-newBgColorInner flex flex-1 relative">
        <div className="absolute top-0 start-0 w-full h-full p-[20px] overflow-auto scrollbar scrollbar-thumb-fifth scrollbar-track-newBgColor">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-[12px] mb-[16px]">
            <div className="flex flex-col gap-[4px]">
              ...
              <div className="text-[12px] text-textItemBlur">
                {selectedIntegration
                  ? `${selectedIntegration.name} - Threads`
                  : 'Select a Threads channel to view replies'}
          ...
          {!selectedIntegration && (
            <div className="text-sm text-textItemBlur">
              Choose a Threads channel from the left panel.
            </div>
          )}

          {selectedIntegration && isLoading && (
            <div className="text-sm text-textItemBlur">Loading replies...</div>
          )}

          {selectedIntegration && !isLoading && !posts.length && (
            <div className="text-sm text-textItemBlur">
